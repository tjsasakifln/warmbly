...
func TestColdReadinessCeilingGraduatesFivePerDay(t *testing.T) {
	...
	if got := coldReadinessCeiling(account, now, 0, 280); got != 50 {
		t.Fatalf("coldReadinessCeiling() = %d, want 50", got)
	}
	if got := coldReadinessCeiling(account, now, 1, 280); got != 38 {
...
func TestColdReadinessCeilingStartsFreshMailboxLow(t *testing.T) {
	...
	if got := coldReadinessCeiling(account, time.Now(), 0, 0); got != 10 {
		t.Fatalf("coldReadinessCeiling() = %d, want 10", got)
	}
}

func TestColdReadinessCeilingRequiresActualWarmupVolume(t *testing.T) {
	now := time.Date(2026, 8, 25, 12, 0, 0, 0, time.UTC)
	warmupStarted := now.Add(-30 * 24 * time.Hour)
	account := models.Email{
		CampaignLimit:  50,
	...
	if got := coldReadinessCeiling(account, now, 0, 0); got != 10 {
		t.Fatalf("coldReadinessCeiling() = %d, want 10 without actual warmup sends", got)
	}
}
