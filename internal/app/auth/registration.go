...
func (s *authService) RegistrationStart(ctx context.Context, data *AuthData, ipaddr, userAgent string) (*models.AuthSession, *errx.Error) {
	...
	if !s.policy.RequireEmailVerification || !s.mailDelivers {
		if err := s.createAccount(ctx, data.Email, passwordHash, data.ReferralCode, signup); err != nil {
			return nil, err
		}
...
func (s *authService) RegistrationConfirm(ctx context.Context, data *ConfirmData, session, ipaddr string) *errx.Error {
	...
	return s.createAccount(ctx, token.Email, sess.PasswordHash, sess.ReferralCode, sess)
}
