...
func fillDelegatedRehearsalRunway(ctx context.Context, svc *service, maxEvaluations int) (int, time.Duration, error) {
	...
	for processed < maxEvaluations {
		if processed%25 == 0 {
			if err := refreshRehearsalMailboxHealth(ctx, svc.delegatedDB, svc.cfg.OperatorOrgID); err != nil {
...
func requireDisposableRehearsalDB(t *testing.T) {
	t.Helper()
	if strings.TrimSpace(os.Getenv("WARMBLY_REHEARSAL_DISPOSABLE_DB")) != "1" {
		t.Skip("WARMBLY_REHEARSAL_DISPOSABLE_DB=1 is required for the destructive opt-in rehearsal tenant")
	}
}

func readRehearsalManifest(t *testing.T, path string) outreachManifest {
	t.Helper()
	raw, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
	var manifest outreachManifest
	if err := json.Unmarshal(raw, &manifest); err != nil {
		t.Fatalf("invalid rehearsal manifest %s: %v", path, err)
	}
	return manifest
}

func prepareRehearsalManifest(t *testing.T, sourcePath string) (string, outreachManifest) {
	t.Helper()
	...
	if err := json.Unmarshal(raw, &source); err != nil {
		t.Fatalf("invalid rehearsal manifest %s: %v", sourcePath, err)
	}
	manifest := readRehearsalManifest(t, sourcePath)
	if manifest.SourceFreshness == nil {
		t.Fatalf("rehearsal manifest %s has no source freshness attestation", sourcePath)
	}
	if strings.TrimSpace(manifest.SourceFreshness.AsOf) == "" {
		freshness, _ := source["authoritative_source_freshness"].(map[string]any)
		if scope, _ := freshness["scope"].(string); scope != "SYNTHETIC_REHEARSAL_ONLY" {
			t.Fatalf("rehearsal-only observed_at compatibility requires SYNTHETIC_REHEARSAL_ONLY scope")
		}
	...
	for index := range manifest.Chunks {
		manifest.Chunks[index].File = "file://" + filepath.Join(baseDir, filepath.Base(manifest.Chunks[index].File))
	}
	prepared, err := json.Marshal(manifest)
	if err != nil {
		t.Fatal(err)
	}
	preparedPath := filepath.Join(t.TempDir(), "manifest.json")
	if err := os.WriteFile(preparedPath, prepared, 0o600); err != nil {
		t.Fatal(err)
	}
	return preparedPath, manifest
}

func refreshRehearsalMailboxHealth(ctx context.Context, pool *pgxpool.Pool, orgID uuid.UUID) error {
	if _, err := pool.Exec(ctx, `
		UPDATE workers w SET last_seen_at=now()
		FROM email_accounts ea
		WHERE ea.organization_id=$1 AND ea.worker_id=w.id`, orgID); err != nil {
		return err
	}
	_, err := pool.Exec(ctx, `
		UPDATE email_accounts
		SET auth_state='passing',auth_spf=true,auth_dkim=true,auth_dmarc=true,auth_checked_at=now()
		WHERE organization_id=$1`, orgID)
	return err
}

func currentRehearsalRows(ctx context.Context, pool *pgxpool.Pool, orgID uuid.UUID, runID string) (map[string]int, error) {
	keys := []string{"accounts", "candidates", "evidence", "initial_touchpoints", "decisions", "queue_live", "queue_cancelled", "smtp_attempted", "provider_accepted", "sent_touchpoints"}
	...
	err := pool.QueryRow(ctx, `SELECT
		...
		(SELECT count(*) FROM confenge_dispatch_reservations WHERE organization_id=$1 AND attempted_at IS NOT NULL),
		(SELECT count(*) FROM confenge_dispatch_sends WHERE organization_id=$1),
...
func TestVersionedScaleManifestQueuesWithoutTransport(t *testing.T) {
	manifestPath := strings.TrimSpace(os.Getenv("WARMBLY_REHEARSAL_CANARY_MANIFEST"))
	if manifestPath == "" {
		t.Skip("WARMBLY_REHEARSAL_CANARY_MANIFEST is not set")
	}
	requireDisposableRehearsalDB(t)
	...
	var manifest outreachManifest
	if err := json.Unmarshal(raw, &manifest); err != nil {
		t.Fatalf("invalid canary manifest: %v", err)
	}
	...
	cfg.DelegatedFirstTouchRunwayDays = 1
	svc := restartDelegatedRehearsalService(f, cfg)
	result, xerr := svc.SyncFeedManifest(f.ctx, f.orgID, &f.actorID, "file://"+manifestPath)
	if xerr != nil || result == nil || result.Status != "completed" ||
		result.Counts["imported"] != manifest.LeadCount ||
		result.Counts["delegated_eligible"]+result.Counts["held_exception"] != manifest.LeadCount {
		t.Fatalf("canary manifest failed: result=%+v err=%v", result, xerr)
	}
	if _, _, err := fillDelegatedRehearsalRunway(f.ctx, svc, manifest.LeadCount); err != nil {
		t.Fatal(err)
	}
	var queued, attempted, accepted, sent int
	if err := f.pool.QueryRow(f.ctx, `SELECT
		...
		Scan(&queued, &attempted, &accepted, &sent); err != nil {
		t.Fatal(err)
	}
	if queued < 1 || attempted != 0 || accepted != 0 || sent != 0 {
		var blockers []byte
		...
		t.Fatalf("versioned corpus canary did not queue safely: queued=%d attempted=%d accepted=%d sent=%d blockers=%s", queued, attempted, accepted, sent, blockers)
	}
}
...
func TestConfengeTenThousandNoSMTPRefreshRecovery(t *testing.T) {
	...
	if canonicalWorkers == 0 {
		...
		workerID := uuid.New()
		if _, err := pool.Exec(ctx, `INSERT INTO workers(id,name,ip_addr,active,worker_type,free_tier,last_seen_at) VALUES($1,'rehearsal-recovery','127.0.0.1',true,'shared',false,now())`, workerID); err != nil {
	...
	if err := refreshRehearsalMailboxHealth(ctx, pool, orgID); err != nil {
		t.Fatalf("refresh synthetic mailbox health: %v", err)
	}

	cfg := Config{
		AppEnv: "test", Enabled: true, DelegatedFirstTouchEnabled: true,
		DelegatedFirstTouchAutorunEnabled: true, DelegatedFirstTouchRunwayDays: 30,
	...
	if xerr != nil || result == nil || (result.Status != "completed" && result.Status != "noop") ||
		(entryWasPartial && (result.ChunksImported != recoveryManifest.ChunkCount || result.Deactivations != recoveryManifest.DeactivationCnt)) ||
		result.Counts["imported"] != recoveryManifest.LeadCount ||
		result.Counts["delegated_eligible"]+result.Counts["held_exception"] != recoveryManifest.LeadCount {
	...
	for replay := 0; replay < 10; replay++ {
		...
		if xerr != nil || readback == nil || !readback.SkippedSame || readback.Counts["imported"] != recoveryManifest.LeadCount {
	...
	metrics := svc.delegatedFirstTouchRunwayMetrics(ctx, orgID)
	assertions := map[string]bool{
		...
		"runway_refilled":               metrics.CapacityBlocked == 0 && rows["queue_live"] == metrics.TargetScheduledCount,
		"runway_no_busy_loop":           idleDuration < 10*time.Second,
		"no_smtp_no_sent":               rows["smtp_attempted"] == 0 && rows["provider_accepted"] == 0 && rows["sent_touchpoints"] == 0,
	...
	report := map[string]any{
		"schema_version": "warmbly.confenge.no-smtp-scale-recovery.v2",
		"status":         "PASS", "no_smtp": true, "dispatch_worker_invoked": false,
		...
		"scheduler": map[string]any{"runway_days": cfg.DelegatedFirstTouchRunwayDays, "runway_target": metrics.TargetScheduledCount, "evaluations": fillCount,
...
func TestConfengeTenThousandNoSMTPRehearsal(t *testing.T) {
	...
	manifestV1, v1Manifest := prepareRehearsalManifest(t, manifestV1)
	manifestV2, v2Manifest := prepareRehearsalManifest(t, manifestV2)
	if v1Manifest.LeadCount != 10_000 || v2Manifest.LeadCount != 10_000 {
		t.Fatalf("10k rehearsal requires two 10k manifests: v1=%d v2=%d", v1Manifest.LeadCount, v2Manifest.LeadCount)
	}
	...
	cfg.DelegatedFirstTouchAutorunEnabled = true
	cfg.DelegatedFirstTouchRunwayDays = 15
	svc := restartDelegatedRehearsalService(f, cfg)
	...
	for partialAccounts == 0 {
		select {
		case early := <-faultDone:
			killImporter()
			t.Fatalf("importer stopped before its first persisted account: result=%+v err=%v", early.result, early.xerr)
		case <-killDeadline.C:
			killImporter()
	...
	if xerr != nil || v1 == nil || v1.Status != "completed" || v1.ChunksImported != v1Manifest.ChunkCount || v1.Counts["imported"] != v1Manifest.LeadCount ||
		v1.Counts["delegated_eligible"]+v1.Counts["held_exception"] != v1Manifest.LeadCount {
	...
	for replay := 0; replay < 2; replay++ {
		...
		if xerr != nil || readback == nil || !readback.SkippedSame || readback.Counts["imported"] != v1Manifest.LeadCount {
	...
	if err := refreshRehearsalMailboxHealth(f.ctx, f.pool, f.orgID); err != nil {
		t.Fatal(err)
	}
	...
	var queuedHalf int
	halfMetrics := restarted.delegatedFirstTouchRunwayMetrics(f.ctx, f.orgID)
	if err := f.pool.QueryRow(f.ctx, `SELECT count(*) FROM confenge_dispatch_queue WHERE organization_id=$1 AND status IN ('queued','reserved')`, f.orgID).Scan(&queuedHalf); err != nil ||
		halfMetrics.CapacityBlocked != 0 || queuedHalf < 1 || queuedHalf != halfMetrics.TargetScheduledCount {
		t.Fatalf("first rolling runway=%d metrics=%+v err=%v", queuedHalf, halfMetrics, err)
	}
	...
	var queuedFull int
	fullMetrics := restartedWorker.delegatedFirstTouchRunwayMetrics(f.ctx, f.orgID)
	if err := f.pool.QueryRow(f.ctx, `SELECT count(*) FROM confenge_dispatch_queue WHERE organization_id=$1 AND status IN ('queued','reserved')`, f.orgID).Scan(&queuedFull); err != nil ||
		fullMetrics.CapacityBlocked != 0 || queuedFull <= queuedHalf || queuedFull != fullMetrics.TargetScheduledCount {
		t.Fatalf("expanded rolling runway=%d first=%d metrics=%+v err=%v", queuedFull, queuedHalf, fullMetrics, err)
	}
	...
	if xerr != nil || v2 == nil || v2.Status != "completed" || v2.ChunksImported != v2Manifest.ChunkCount || v2.Deactivations != v2Manifest.DeactivationCnt ||
		v2.Counts["imported"] != v2Manifest.LeadCount || v2.Counts["delegated_eligible"]+v2.Counts["held_exception"] != v2Manifest.LeadCount {
	...
	for replay := 0; replay < 10; replay++ {
		...
		if xerr != nil || readback == nil || !readback.SkippedSame || readback.Counts["imported"] != v2Manifest.LeadCount {
	...
	refillMetrics := restartedWorker.delegatedFirstTouchRunwayMetrics(f.ctx, f.orgID)
	if err != nil || rowsV2["accounts"] != 10_000 || refillMetrics.CapacityBlocked != 0 ||
		rowsV2["queue_live"] != refillMetrics.TargetScheduledCount || rowsV2["smtp_attempted"] != 0 ||
		rowsV2["provider_accepted"] != 0 || rowsV2["sent_touchpoints"] != 0 {
	...
	queueThroughput := float64(queuedFull) / queueFillDuration.Seconds()
	assertions := map[string]bool{
		...
		"no_smtp_no_sent":               rowsV2["smtp_attempted"] == 0 && rowsV2["provider_accepted"] == 0 && rowsV2["sent_touchpoints"] == 0,
	...
	report := map[string]any{
		...
		"scheduler": map[string]any{
			"initial_runway_days": 15, "expanded_runway_days": 30,
			"runway_target": refillMetrics.TargetScheduledCount, "evaluations_before_restart": evaluatedBeforeRestart,
