...
`GET /confenge/first-touch/status` returns delegated decision readback plus the `runway` capacity snapshot. The snapshot includes the prepared reservoir, separate queued and reserved counts, capacity-derived target count and horizon, furthest due time, mailbox and daily capacity, recent fill rate, stale retirements, holds and fail-closed blocker signals. It never turns queue depth into send rate.
