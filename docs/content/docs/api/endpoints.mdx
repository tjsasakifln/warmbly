...
`GET /confenge/intel/outcome-feedback` returns the read-only `confenge.acquisition_outcome_feedback.v1` aggregate for inclusive `from` / `to` UTC cohort months (`YYYY-MM`, current plus previous two months by default). It separates receipts, leads, observed QCOs, chain-projected proposals, human-confirmed outcomes, contracted/received value, contracts, and margin. A validated lead is not QCO. Missing contract or margin evidence stays `UNJOINABLE` / `UNKNOWN`, never zero. Direct closed-context/month/record-kind cells below five stable account privacy units are combined into a dimensionless roll-up; if that roll-up is also below five, its counts and values are null. QCO, proposal, WON/LOST, contracted value, and received value each require five contributors independently. Synthetic and real thresholds never mix. Raw landing path, asset ID, CTA ID, intent class, and buyer job stay `UNJOINABLE` / `UNKNOWN` until closed versioned registries exist. The response contains no lead/receipt/correlation/account/person/payment identity or free text, reads only existing chains, and never writes upstream.
