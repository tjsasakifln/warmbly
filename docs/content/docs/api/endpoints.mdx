...
`GET /confenge/status` reports authoritative feed age separately from sync time. For delegated first touch, `feed_authority_state` becomes `expired` when the producer-owned `feed_source_expires_at` passes even if the generic 24-hour age is still fresh. The readiness payload also exposes `target_membership_complete`, `target_membership_count`, and `supplier_confirmed_count`; incomplete authority remains fail-closed.
