...
The mailbox's own cold graduation is another minimum. A fresh mailbox starts cold sending at `10`/day, or at its proven average warmup volume from the last seven days, then gains `5`/day. Recent spam-folder placement cuts that daily ceiling by `25%`. Campaign settings can lower the result but cannot bypass it.
