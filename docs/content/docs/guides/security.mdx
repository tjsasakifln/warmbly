...
Password registration also evaluates disposable-address signals, repeated normalized aliases, signup IP and ASN velocity, and hosting-network signals after CAPTCHA succeeds. Warmbly stores the signup IP, user agent, email risk score, and ASN for abuse correlation. These signals do not reveal another account's identity, but they can move a new workspace into monitored or restricted sending until it establishes normal behavior.
